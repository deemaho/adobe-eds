...
function decorateSections(main) {
	[...main.querySelectorAll(':scope > div')].forEach((section) => {
		...
		// Move attributes from div to section tag
		[...section.attributes].forEach((attr) => sectionTag.setAttribute(attr.name, attr.value));

		// Check if div has an ID
		if (section.hasAttribute('id')) sectionTag.id = section.id;

		// Move child elements from div to section tag
		while (section.firstChild) sectionTag.appendChild(section.firstChild);

		// Replace div with section tag
		section.parentNode.replaceChild(sectionTag, section);
	});
}
